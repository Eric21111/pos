...
const run = async () => {
  try {
    ...
    if (existing) {
      ...
      existing.pin = '123456'; // 6 digits required by schema
      ...
      await existing.save();
      console.log('🔐 Owner PIN reset to 123456 (hashed in DB).');
    } else {
      ...
      await Employee.create({
        ...
        email: OWNER_EMAIL,
        role: 'Owner',
        pin: '123456', // 6 digits required by schema
        status: 'Active',
        profileImage: '',
        permissions: {
          posTerminal: true,
      ...
      console.log('✅ Owner account created with PIN 123456.');
    }
  } catch (error) {
