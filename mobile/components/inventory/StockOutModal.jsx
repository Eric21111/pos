...
const StockOutModal = ({ visible, onClose, product, onConfirm, loading }) => {
    ...
    const handleQuantityChange = (size, qty) => {
        ...
        const maxQty = product.sizes && product.sizes[size] ? getSizeQuantity(product.sizes[size]) : 0;
    ...
    const handleSubmit = () => {
        if (availableSizes.length === 0) {
            Alert.alert('Error', 'This product has no sizes available with stock');
            return;
        }
        ...
        if (!hasValidQuantities) {
            Alert.alert('Error', 'Please enter quantities for at least one selected size');
            return;
        }
        ...
        if (invalidSizes.length > 0) {
            Alert.alert('Error', `Cannot remove more than available stock for: ${invalidSizes.join(", ")}`);
            return;
        }
                ...
                <View style={styles.modalContainer}>
                    ...
                    <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
                        ...
                        <View style={styles.productInfo}>
                            {product.itemImage ? (
                                <Image source={{ uri: product.itemImage }} style={styles.productImage} resizeMode="contain" />
                            ) : (
                        ...
                        <View style={styles.section}>
                            <Text style={styles.sectionTitle}>Select Sizes to Remove</Text>
                            {availableSizes.length > 0 ? (
                                ...
                                <Text style={styles.noStockText}>No sizes available with stock.</Text>
                            )}
                        </View>
                                    ...
                                    return (
                                        <View key={size} style={styles.quantityRowContainer}>
                                            <View style={styles.quantityRow}>
                                                ...
                                                <TextInput
                                                    style={[styles.quantityInput, isError && styles.inputError]}
                                                    value={sizeQuantities[size]?.toString() || ''}
                                                    onChangeText={(text) => handleQuantityChange(size, text)}
                                                    placeholder="0"
                                                    keyboardType="numeric"
                                                />
                                            </View>
                                            {isError && (
                                                <Text style={styles.errorText}>Cannot exceed {maxQty}</Text>
                                            )}
                                        </View>
                                    );
                                })}
